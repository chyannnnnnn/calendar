...
function loadGoogleMaps(apiKey) {
  ...
  _mapsPromise = new Promise((resolve, reject) => {
    ...
    if (existing) { existing.onload = () => resolve(window.google.maps); return }
    const script = document.createElement('script')
    script.setAttribute('data-gm','1')
    script.src = `https://maps.googleapis.com/maps/api/js?key=${apiKey}&libraries=places`
    script.async = true
    script.onload  = () => resolve(window.google.maps)
    script.onerror = () => { _mapsPromise = null; reject(new Error('Google Maps failed to load. Check your API key.')) }
...
export default function LocationPicker({ value, onChange, apiKey, readOnly = false }) {
  ...
  // Load SDK when picker opens
  useEffect(() => {
    if (!open || loaded || !apiKey) return
    loadGoogleMaps(apiKey)
      .then(() => setLoaded(true))
      .catch(e => setLoadErr(e.message))
  }, [open, apiKey, loaded])

  // Init map once SDK loaded
  useEffect(() => {
    ...
    if (value?.lat) {
      marker.current = new G.Marker({ position: center, map, draggable: true })
      marker.current.addListener('dragend', e => reverseGeocode(e.latLng.lat(), e.latLng.lng()))
    }
    map.addListener('click', e => dropPin(e.latLng.lat(), e.latLng.lng()))
  }, [loaded, open])

  function isDark() { return C.bg !== '#FDF6EE' }
  ...
  const fetchSuggestions = useCallback((input) => {
    ...
    acService.current.getPlacePredictions(
      ...
      (preds, status) => {
        setSugs(status === window.google.maps.places.PlacesServiceStatus.OK && preds ? preds : [])
      }
    )
  }, [])

  useEffect(() => {
    const t = setTimeout(() => fetchSuggestions(query), 260)
  ...
  function selectSuggestion(placeId, description) {
    ...
    geocoder.current.geocode({ placeId }, (results, status) => {
      if (status === 'OK' && results[0]) {
        const { lat, lng } = { lat: results[0].geometry.location.lat(), lng: results[0].geometry.location.lng() }
        dropPin(lat, lng)
  ...
  function openPicker() {
    mapInstance.current = null  // reset so map re-inits with fresh ref
    setOpen(true)
    setQuery(value?.name || '')
    setSugs([])
  }

  // READ-ONLY: clickable row that opens Google Maps
  if (readOnly) {
    ...
    return (
      <a href={mapsUrl} target="_blank" rel="noopener noreferrer"
        style={{
          ...
          textDecoration:'none',border:`1px solid ${C.border}`,transition:'all 0.2s',cursor:'pointer',
        }}
      ...
      >
        ...
        <span style={{fontSize:11,fontWeight:700,color:C.peach,background:C.peach+'18',border:`1px solid ${C.peach}44`,borderRadius:20,padding:'2px 10px',flexShrink:0}}>
          ↗ Maps
        </span>
      </a>
    )
  ...
  return (
    <>
      ...
      >
        ...
        {value
          ? <button onClick={clearLocation} style={{background:'none',border:'none',color:C.textDim,cursor:'pointer',fontSize:16,padding:'0 2px',flexShrink:0,lineHeight:1}}>✕</button>
          : <span style={{fontSize:11,color:C.peach,fontWeight:700,flexShrink:0}}>Pin 📌</span>
        }
      </div>
          ...
          <div style={{
            ...
            <div style={{padding:'16px 18px 10px',borderBottom:`1px solid ${C.border}`,flexShrink:0}}>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:10}}>
                <div style={{fontFamily:"'Playfair Display'",fontSize:18,color:C.text,display:'flex',alignItems:'center',gap:8}}>
                  📍 Pick a location
                </div>
              ...
              {!apiKey ? (
                <div style={{background:C.rose+'18',border:`1px solid ${C.rose}44`,borderRadius:10,padding:'10px 13px',fontSize:12,color:C.rose}}>
                  ⚠️ Add <code>VITE_GOOGLE_MAPS_KEY</code> to your <code>.env</code> and Vercel env vars to enable map search.
                </div>
              ) : (
                      ...
                      {suggestions.map((s, i) => (
                        ...
                        >
                          ...
                          <div style={{minWidth:0}}>
                            ...
                            <div style={{fontSize:11,color:C.textDim,marginTop:1,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>
              ...
              <div style={{fontSize:11,color:C.textDim,marginTop:8}}>
                Or tap anywhere on the map to drop a pin 📌 • Drag pin to adjust
              </div>
            </div>
            ...
            <div style={{flex:1,minHeight:280,position:'relative'}}>
              ...
              {(!loaded && apiKey && !loadErr) && (
                <div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',background:C.bg,color:C.textDim,fontSize:13,gap:8}}>
                  <span style={{animation:'spin 1s linear infinite',display:'inline-block'}}>✿</span> Loading map…
                </div>
              )}
              {loadErr && (
                <div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',background:C.bg,color:C.rose,fontSize:13,padding:20,textAlign:'center'}}>
                  ⚠️ {loadErr}
                </div>
              )}
            </div>
            ...
            <div style={{padding:'12px 18px',borderTop:`1px solid ${C.border}`,flexShrink:0,display:'flex',alignItems:'center',gap:10}}>
              <div style={{flex:1,fontSize:12,color:C.textMid,lineHeight:1.4}}>
                {value ? <><span style={{color:C.mint,marginRight:6}}>✅</span>{value.name}</> : <span style={{color:C.textDim}}>No location pinned yet</span>}
              </div>
              {value && (
